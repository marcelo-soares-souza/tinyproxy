...
static struct upstream *upstream_build (const char *user, const char *pwd, const char *host, int port, const char *domain)
{
        ...
        if (domain == NULL) {
                ...
                log_message (LOG_INFO, "Added upstream %s:<pwd>@%s:%d for [default]",
...
char *upstream_proxy_auth_user_sub (const char *user)
{

    char *new_user = NULL;

    if (strstr(user, "{{RANDINT}}") != NULL) {

        char rand_str[64];
        sprintf(rand_str, "%d", rand());

        new_user = repl_str(user, "{{RANDINT}}", rand_str);
    }
